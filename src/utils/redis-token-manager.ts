...
export class CompanyTokenManager {
  ...
  async priorityTokens(transferSeriesId?: string): Promise<ITokenData[]> {
    ...
    let filterSeries;
    if (transferSeriesId) {
       filterSeries = await prisma.ht_series.findUniqueOrThrow({
        where: {
          hash_id: transferSeriesId,
          company: {
            hash_id: this.param.company_id
          },
          deleted_at: null
    ...
    const filteredTokens = tokenListData.filter(token =>
      ...
      ( filterSeries ? filterSeries.abbreviation === token.token_abbreviation : allowedAbbreviations.includes(token.token_abbreviation)) &&
