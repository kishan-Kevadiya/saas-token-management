...
export default class TokenService {
  ...
  private applyTransferTokenFilter(
    ...
    return tokens.filter((token) => {
      ...
      if (counter.transfer_token_wise === TransferTokenWise.COUNTER) {
        ...
        return counter.id === token.transfer_counter?.id;
      } else if (counter.transfer_token_wise === TransferTokenWise.DEPARTMENT) {
        const currentDepartmentId = counter.dept_id;
        return currentDepartmentId === token.transfer_department?.id;
      }

      // For other transfer_token_wise values or undefined cases, include the token
      return true;
    });
  }
